...
class KisilerDaoRepository {
    ...
    var collectionKisiler = Firestore.firestore().collection("Kişiler")
    ...
    func guncelle(kisi_ad:String,kisi_tel:String,kisi_id:String) {
        let yeniKisi:[String:Any] = ["kisi_ad":kisi_ad,"kisi_tel":kisi_tel]
        collectionKisiler.document(kisi_id).updateData(yeniKisi)
    }
            ...
            if let document = snapshot?.documents {
                for doc in document {
                    ...
                    if kisi_ad.lowercased().contains(search.lowercased()) {
                        let kisi = Kisiler(kisi_id: id, kisi_ad: kisi_ad, kisi_tel: kisi_tel)
                        liste.append(kisi)
                    }
                }
            }
